...
const reveals=[...document.querySelectorAll('.reveal')];const observer=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting)e.target.classList.add('in')}),{threshold:.14});reveals.forEach(e=>observer.observe(e));
...
// Leadership AI embedded product readiness and in-place recovery
const leadershipFrame=document.querySelector('#leadershipFrame'),leadershipLoading=document.querySelector('#leadershipLoading'),leadershipStatus=document.querySelector('#leadershipStatus'),reloadLeadership=document.querySelector('#reloadLeadership');
if(leadershipFrame&&leadershipLoading&&leadershipStatus){
  let readyTimer,stallTimer;
  const setLoading=()=>{leadershipLoading.classList.remove('ready','stalled');leadershipStatus.textContent='Loading product…'};
  const checkLeadership=(finalCheck=false)=>{try{const d=leadershipFrame.contentDocument,text=d?.body?.innerText||'';if(text.length>500&&(text.includes('Daily Vanguard')||text.includes('What requires my attention today?')||text.includes('Execution Tracking'))){leadershipLoading.classList.add('ready');leadershipLoading.classList.remove('stalled');leadershipStatus.textContent='Product ready';return true}}catch{}if(finalCheck){leadershipLoading.classList.add('stalled');leadershipStatus.textContent='Reload if needed';const note=leadershipLoading.querySelector('span');if(note)note.textContent=location.protocol==='file:'?'Serve this folder over HTTP to load the interactive product.':'The product did not finish loading. Retry here without leaving the page.'}return false};
  const beginLeadershipCheck=()=>{clearTimeout(readyTimer);clearTimeout(stallTimer);setLoading();readyTimer=setTimeout(()=>checkLeadership(false),900);stallTimer=setTimeout(()=>checkLeadership(true),6500)};
  leadershipFrame.addEventListener('load',beginLeadershipCheck);
  reloadLeadership?.addEventListener('click',()=>{const base='demos/leadership-ai/daily-vanguard/index.html';leadershipFrame.src=`${base}?embedded=4&reload=${Date.now()}`});
